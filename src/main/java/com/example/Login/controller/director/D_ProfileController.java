...
@Controller
public class D_ProfileController {
    ...
    @PreAuthorize("hasRole('DIRECTOR')")
    @PostMapping("/director/profile/photo")
    public String uploadProfilePhoto(@RequestParam("photo") MultipartFile photo, Authentication authentication) {
        User user = getCurrentUser(authentication);
        if (user == null) {
            return "redirect:/director/profile?error=User+not+found";
        }
        if (photo == null || photo.isEmpty()) {
            return "redirect:/director/profile?error=No+file+selected";
        }
        String contentType = photo.getContentType();
        if (contentType == null || !contentType.startsWith("image/")) {
            return "redirect:/director/profile?error=Only+image+files+are+allowed";
        }
        try {
            ...
            return "redirect:/director/profile?success=Profile+photo+updated";
        } catch (IOException e) {
            return "redirect:/director/profile?error=Failed+to+save+file";
        }
    }

    @PreAuthorize("hasRole('DIRECTOR')")
    @PostMapping("/director/profile/photo/delete")
    public String deleteProfilePhoto(Authentication authentication) {
        ...
        userRepository.save(user);
        return "redirect:/director/profile?success=Profile+photo+removed";
    }
}
